...
void myRectangle(int x1, int y1, int x2, int y2, int R, int G, int B, int R2, int G2, int B2)
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);
    ...
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R2, G2, B2)); //Fill color can also be passed as parameter to the function!!!
...
void myEllipse(int x1, int y1, int x2, int y2, int R, int G, int B, int R2, int G2, int B2)
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);
    ...
    HPEN pen =CreatePen(PS_SOLID,2,RGB(R,G,B)); 
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(R2, G2, B2));  //Fill color is black
	SelectObject(device_context,brush);
